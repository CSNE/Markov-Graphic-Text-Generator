...
from SimpleMaths import linearMap
...
class MarkovNode():
    ...
    def jump(self):
        '''
        Jumps from one node to the other.
        The node that got jumped is returned.
        This node is then deactivated, and the jumped node is activated.
        '''
        nxt = self.select()
        return nxt
...
class MarkovDraw:
        ...
        for destination_node in targets:  # create a new line
            self.line_ids[destination_node] = self.canvas.create_line(
                ...
                width=self.connections_to_width(self.node.destination_nodes.count(destination_node.node),
                                                self.node.get_max_connections()))  # TODO Better Algorithm for determining line width
    ...
    def update(self, current_time):
        ...
        x = int(self.animated_x.getValue(current_time))
        y = int(self.animated_y.getValue(current_time))
...
def elements_to_tuple(lst, mode):
    res = list()
    for i in range(len(lst)):
        if mode == 'Word':
...
def generate_chain(lst, mode):
    ...
    for i in range(len(lst)):
        ...
        mn = None  # Is this element already in the list of nodes?
...
def follow_node():
    ...
    global active_node, nodes, chain_results_box, to_be_active, nodes_draw
    global canvas
    ...
    # First step
    if active_node == None:
        to_be_active = nodes[0]
    else:

        try:

            to_be_active = active_node.node.jump()
            for i in nodes_draw:
    ...
    canvas.after(int(linearMap(0, 100, 0, 1500, generate_delay)), follow_node_part2)
...
def follow_node_part2():
    ...
    active_node.move_to(center[0], center[1], (linearMap(0, 100, 0, 1.5, generate_delay)), True, True)
    ...
    for i in range(len(destination_nodes)):
        ...
        nodes_draw[-1].move_to(others[i][0], others[i][1], (linearMap(0, 100, 0, 1.5, generate_delay)), True, True)
    ...
    if generate:
        tk.after(int(linearMap(0, 100, 0, 3000, generate_delay)), follow_node)
...
# Tk
tk = Tk()
tk.title("Markov Chain Visualizer")
...
# Tk>Canvas
canvas = Canvas(tk, background="#FFFFFF")
